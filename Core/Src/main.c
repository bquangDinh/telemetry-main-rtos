...
static void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB, CAN_TX_LED_Pin | CAN_RX_LED_Pin, GPIO_PIN_RESET);

	/*Configure GPIO pin : NOTE_CARD_ATTN_Pin */
	GPIO_InitStruct.Pin = NOTE_CARD_ATTN_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(NOTE_CARD_ATTN_GPIO_Port, &GPIO_InitStruct);

	/*Configure GPIO pin : CAN_ERR_LED_Pin */
	GPIO_InitStruct.Pin = CAN_ERR_LED_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(CAN_ERR_LED_GPIO_Port, &GPIO_InitStruct);

	/*Configure GPIO pins : CAN_TX_LED_Pin CAN_RX_LED_Pin */
	GPIO_InitStruct.Pin = CAN_TX_LED_Pin | CAN_RX_LED_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	...
	HAL_NVIC_EnableIRQ(NOTE_CARD_ATTN_EXTI_IRQn);
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
	if (huart->Instance == WIFI_ESP32_UART_INSTANCE) {
		WIFI_esp32_uart_rx_callback(Size);
	} else if (huart->Instance == CELLULAR_BLUES_UART_INSTANCE) {
		CELLULAR_blues_uart_rx_callback(Size);
	}
}

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin == CELLULAR_BLUES_ATTN_GPIO_PIN) {
		CELLULAR_ATTN_callback();
	}
}

void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs) {
//	if (hfdcan->Instance == CAN_CONTROLLER_CAN_INSTANCE) {
//
//	}
	CAN_CONTROLLER_rx_callback(RxFifo0ITs);
}
...
//
//	}
	CAN_CONTROLLER_tx_callback(BufferIndexes);
}
/* USER CODE END 4 */
